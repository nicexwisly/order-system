...
export default function ButcheryDepartmentPage() {
  ...
  const loadOrders = async () => {
    try {
      setLoading(true)
      const data = await orderService.getOrdersByDepartment("Butchery")
      setOrders(data)
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Butchery Department</h1>
            <p className="text-gray-600 mt-2">Manage butchery orders and update status</p>
          </div>
        ...
        <div className="space-y-4">
          {orders.length === 0 ? (
            <div className="card text-center py-12">
              <p className="text-gray-500 text-lg">No butchery orders found</p>
            </div>
          ) : (
            orders.map((order) => (
